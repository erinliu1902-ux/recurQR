...
# ─── Data Generation ────────────────────────────────────────────────────────────
# Generates recurrent-event data for n subjects up to subject-specific censoring times.
# Args:
#   n     – number of subjects
#   max.C – numeric vector of length n giving each subject’s maximum follow-up time
# Returns:
#   data.frame with one row per observed episode, including episode times,
#   covariates z1, z2, time-dependent count z3, and derived quantities.
data.genrt <- function(n, max.C) {
  ...
  for (k in 1:n) {
    ...
    while (start < max.C[k]) {
      ...
      curr.z3 <- indx
      Xt <- 10 + (d.int - z1 + z2)[k] + 0.4 * curr.z3 + rnorm(1, mean = 0, sd = sqrt(1/2))
      if (end + Xt < max.C[k]) {
        indx <- indx + 1
...
# ─── Censoring/Sample Functions ─────────────────────────────────────────────────
# Empirical survival function at time x based on vector fix.C
S.C <- function(x, fix.C) {
  return(length(which(fix.C >= x)) / length(fix.C))
}

# Generate n normal variates with mean mu, sd sd, truncated below at min
...
# Performs B bootstrap samples of two-stage quantile regression,
# returns mean, sd, and 95% CI of all coefficients.
# Args:
#   da    – data.frame from data.genrt
#   B     – number of bootstrap replicates
#   fix.C – censoring vector used in weights
# Returns:
#   list of four numeric vectors: mean, sd, lower CI, upper CI
...
# Runs 1,000 replicates of various QR estimators under two sample sizes and
# censoring scenarios. Returns bias/sd table, average event frequency, and truncation.
# Args:
#   i_sim – index (1–4) selecting combination of N = {250, 500} and muC = {25, 50}
# Returns:
#   list(sim.res (12×12 table), sim.freq, sim.trun)
sim.RED <- function(i_sim) {
  ...
  taus      <- c(0.25, 0.5, 0.75)
  true.coef <- t(cbind(10 + qnorm(taus), -1, 1, 0.4))
  ...
  cr <- NULL
  for (i_col in 1:12) {
    cr <- c(cr, coverage.rate2(low = low.w6[, i_col], high = high.w6[, i_col], true = true.coef[i_col]))
  }
  tab1[, 12] <- cr
  colnames(tab1) <- c("True", rep(c("bias", "sd"), 4), "mean bias", "estimated var", "coverage rate")
