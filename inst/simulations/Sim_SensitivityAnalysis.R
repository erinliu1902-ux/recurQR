...
boot.qr <- function(da, B, fix.C) {
  ...
  for (i_B in 1:B) {
    ...
    fit.w3.2 <- rq(chgn_epi_length ~ chgn_z3,
...
sim.RED <- function(i_sim) {
  ...
  for (sim in 1:1000) {
    ...
    f1 <- rq(n_epi_length_1 ~ z1 + z2,
             data = da, tau = taus[1],
             weights = 1 / n_episode)
    f2 <- rq(n_epi_length_2 ~ z1 + z2,
             data = da, tau = taus[2],
             weights = 1 / n_episode)
    f3 <- rq(n_epi_length_3 ~ z1 + z2,
             data = da, tau = taus[3],
             weights = 1 / n_episode)
    ...
    fit.w6.2 <- rq(chgn_epi_length ~ chgn_z3,
                   data = da[da$first == 0, ], tau = taus)
    ...
    w1 <- rq(n_epi_length_1 ~ z1 + z2,
             data = da, tau = taus[1],
             weights = 1 / n_episode * wc1)
    w2 <- rq(n_epi_length_2 ~ z1 + z2,
             data = da, tau = taus[2],
             weights = 1 / n_episode * wc2)
    w3 <- rq(n_epi_length_3 ~ z1 + z2,
             data = da, tau = taus[3],
             weights = 1 / n_episode * wc3)
