...
data.genrt <- function(n, max.C) {
  ...
  for (k in 1:n) {
    ...
    while (start < max.C[k]) {
      ...
      curr.z3 <- indx
      Xt <- 10 + (d.int - z1 + z2)[k] + 0.4 * curr.z3 + 
        rnorm(1, mean = 0, sd = sqrt(1/2))
      if (end + Xt < max.C[k]) {
        indx <- indx + 1
...
# Compute S_C(x) = P(C ≥ x) based on censoring vector fix.C.
# Args:
#   x     – time point
...
# Args:
#   i_sim – index (1–4) selecting N = {250,500} and muC = {25,50}
# Returns:
...
sim.RED <- function(i_sim) {
  ...
  taus      <- c(0.25, 0.5, 0.75)
  true.coef <- t(cbind(10, qnorm(taus) - 1, qnorm(taus) + 1, 0.4))
