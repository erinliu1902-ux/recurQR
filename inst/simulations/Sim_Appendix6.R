...
sim.RED <- function(i_sim) {
  ...
  for (sim in 1:1000) {
    ...
    fit.w4 <- rq(epi_length ~ z1 + z2 + z3, data = da, tau = taus,
    ...
    fit5_1 <- rq(n_epi_length_1 ~ z1 + z2, data = da, tau = taus[1], weights = 1 / n_episode)
    fit5_2 <- rq(n_epi_length_2 ~ z1 + z2, data = da, tau = taus[2], weights = 1 / n_episode)
    fit5_3 <- rq(n_epi_length_3 ~ z1 + z2, data = da, tau = taus[3], weights = 1 / n_episode)
    ...
    fit6_1 <- rq(n_epi_length_1 ~ z1 + z2, data = da, tau = taus[1],
                 weights = 1 / n_episode * wc1)
    fit6_2 <- rq(n_epi_length_2 ~ z1 + z2, data = da, tau = taus[2],
                 weights = 1 / n_episode * wc2)
    fit6_3 <- rq(n_epi_length_3 ~ z1 + z2, data = da, tau = taus[3],
