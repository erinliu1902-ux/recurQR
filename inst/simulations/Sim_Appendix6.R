...
# ─── Data Generation ────────────────────────────────────────────────────────────
# Simulate recurrent-event data for n subjects with subject-specific follow-up.
# Args:
...
#   episode lengths, covariates z1, z2, time-dependent count z3, and derived fields.
data.genrt <- function(n, max.C) {
  ...
  for (k in 1:n) {
    ...
    indx  <- 0
    while (start < max.C[k]) {
      if (indx == 0) ht <- 0 else ht <- obs[indx, 4]
      dW  <- rexp(1, 0.2 * exp(ht * 0.2))
      end <- start + dW
      curr.z3 <- indx
      Xt <- 5 + (d.int - z1 + z2)[k] + curr.z3 + rnorm(1, mean = 0, sd = sqrt(1/2))
      if (end + Xt < max.C[k]) {
        indx <- indx + 1
...
# Compute survival function S_C(x) = P(C ≥ x) from censoring times.
# Args:
#   x     – time point
...
# Perform B bootstrap replicates of two-stage weighted QR estimator.
# Args:
#   da    – data.frame from data.genrt
#   B     – number of bootstrap samples
#   fix.C – censoring vector for computing weights
# Returns:
#   List of: mean coefficients, sd coefficients, lower 95% CI, upper 95% CI
boot.qr <- function(da, B, fix.C) {
  ...
  for (i_B in 1:B) {
    ...
    coef.w3 <- rbind(coef.w3,
                     c(rbind(cbind(fit1$coef, fit2$coef, fit3$coef),
                             fit.w3.2$coef[2, ])))
  }
...
coverage.rate1 <- function(mean, sd, true) {
  res <- as.numeric(true <= mean + 1.96 * sd & true >= mean - 1.96 * sd)
...
# compute naive and weighted QR estimators, and compare to true coefficients.
# Args:
#   i_sim – index selecting combination of N = {250, 500} and muC = {12, 50}
# Returns:
#   List(sim.res: 12×12 matrix of bias/sd results, sim.freq, sim.trun)
sim.RED <- function(i_sim) {
  ...
  taus <- c(0.25, 0.5, 0.75)
  # Define mixture distributions and invert for true quantiles
  dist1    <- function(x) pnorm(x - 1, sd = sqrt(1/2)) * 0.2 +
    pnorm(x, sd = sqrt(1/2)) * 0.8
  dist_inv1 <- function(y) uniroot(function(x) dist1(x) - y, interval = c(-100, 100))$root
  dist2    <- function(x) pnorm(x - 1, sd = sqrt(1/2)) * 0.6 +
    pnorm(x, sd = sqrt(1/2)) * 0.4
  dist_inv2 <- function(y) uniroot(function(x) dist2(x) - y, interval = c(-100, 100))$root
  Q1       <- c(dist_inv1(0.25), dist_inv1(0.5), dist_inv1(0.75))
  Q0       <- c(dist_inv2(0.25), dist_inv2(0.5), dist_inv2(0.75))
  true.coef <- t(cbind(5 + Q0, -1, Q1 - Q0 + 1, 1))
  ...
  for (sim in 1:1000) {
    ...
    fit.w4 <- quantreg::rq(epi_length ~ z1 + z2 + z3, data = da, tau = taus,
    ...
    fit5_1 <- quantreg::rq(n_epi_length_1 ~ z1 + z2, data = da, tau = taus[1], weights = 1 / n_episode)
    fit5_2 <- quantreg::rq(n_epi_length_2 ~ z1 + z2, data = da, tau = taus[2], weights = 1 / n_episode)
    fit5_3 <- quantreg::rq(n_epi_length_3 ~ z1 + z2, data = da, tau = taus[3], weights = 1 / n_episode)
    coef.w5 <- rbind(coef.w5,
                     c(rbind(cbind(fit5_1$coef, fit5_2$coef, fit5_3$coef),
                             fit.w5.2$coef[2, ])))
    ...
    fit6_1 <- quantreg::rq(n_epi_length_1 ~ z1 + z2, data = da, tau = taus[1],
                 weights = 1 / n_episode * wc1)
    fit6_2 <- quantreg::rq(n_epi_length_2 ~ z1 + z2, data = da, tau = taus[2],
                 weights = 1 / n_episode * wc2)
    fit6_3 <- quantreg::rq(n_epi_length_3 ~ z1 + z2, data = da, tau = taus[3],
                 weights = 1 / n_episode * wc3)
    coef.w6 <- rbind(coef.w6,
                     c(rbind(cbind(fit6_1$coef, fit6_2$coef, fit6_3$coef),
                             fit.w6.2$coef[2, ])))
